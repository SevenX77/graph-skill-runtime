...
This document records the implemented top-level Python contract for `graph-skill-runtime` `1.0.0a1`. Phase 2 changed the accepted business-skill format, Phase 3 added bounded durable host-native execution semantics, Phase 4 added direct vendor CLI execution plus the public `AgentResource` model, and Phase 5 added explicit optional-integration planning and installation contracts. Historical Phase 6 evidence accepted one `0.1.0a1` built distribution across three platforms without adding a public Python symbol. The provider-neutral Agent kit and CLI authoring commands likewise add no Python symbol. The executable symbol source is [`graph_skill_runtime.__all__`](../src/graph_skill_runtime/__init__.py); this document must contain exactly one strict `## <symbol>` heading for each of those 77 names. The distribution has not been published to PyPI or TestPyPI. Phase 3b remains outside the implemented contract.
...
The top-level facade exposes fourteen functions. Nine belong to the runtime/application boundary: `create_application` is the explicit composition factory, while the eight functions defined in [`sdk.py`](../src/graph_skill_runtime/sdk.py)—`compile`, `resolve_run`, `predict`, `run`, `resume`, `submit_agent_result`, `inspect`, and `evaluate_golden`—delegate to one [`RuntimeApplication`](../src/graph_skill_runtime/application/service.py), accept an optional `application=`, and otherwise call that factory without a global singleton. Five integration functions in `sdk.py`—`detect_integration_hosts`, `plan_integration_install`, `install_integration`, `plan_integration_uninstall`, and `uninstall_integration`—accept an optional `installer=` and otherwise construct an isolated `IntegrationInstaller`.

The MCP server exposes exactly the eight runtime tools `compile`, `resolve_run`, `predict`, `run`, `resume`, `submit_agent_result`, `inspect`, and `evaluate_golden`. Integration detection and mutation are SDK/module-CLI boundaries, not MCP tools. The distribution intentionally defines no `[project.scripts]` or `console_scripts` entry and installs no package-owned launcher. The CLI fallback is the installed interpreter running `python -m graph_skill_runtime ...`; `python -m graph_skill_runtime --version` prints `python -m graph_skill_runtime gskill.graph.v1 (graph-skill-runtime 1.0.0a1)`. The module CLI additionally owns `guide agent-configuration`, `create`, `migrate`, `integrations`, and `mcp`; none extends the Python facade or MCP surface, and there is no unified-kit setup/install command.

## 2. Phase boundary

The current engine adapter accepts only an explicit business-skill root defined by the [portable format contract](skill-spec/01-PORTABLE-GSKILL-V1.md): root `SKILL.md` with `metadata.gskill: gskill.graph.v1`, `graph.yaml` with `schema_version: gskill.graph.v1`, phase `LOGIC.md` / `AGENT.md` / `SUBGRAPH.md`, and an optional flat `graphs/<graph_id>/` registry. Distribution and syntax majors must match. A missing or mismatched root marker fails with `[F-v3-skill-metadata-invalid]`. Production SDK, module CLI, MCP, compile, inspect, predict, and run paths do not fall back to v0.3. Legacy parsing is confined to the explicit `python -m graph_skill_runtime migrate studio-skill` converter.
...
The provider-neutral Agent kit is a separate packaged instruction boundary defined by the [unified Agent-kit design](design/unified-agent-kit.md). It contains one AGENTS section, standalone rules, and exactly two automatic Skills with complete reference trees. `python -m graph_skill_runtime guide agent-configuration` returns those sources and placement choices as read-only JSON and performs no writes. The owner selects host, scope, rules destination, Skill destinations, and manual versus explicitly authorized Agent editing; any write follows inspection, an additive plan, and approval and never replaces an existing instruction file. On `agent_required`, the current host creates one fresh native clean-context child using the canonical prompt owned only by packaged rule 05; the runtime's typed `AgentTask` and `AgentResult` contracts remain unchanged.

Phase 5 acceptance covers the canonical bundle, six deterministic project/user renderers, explicit installer safety, built-wheel contents and projection, and bounded real-host discovery. Renderer snapshots fix native paths, profile metadata, and MCP shapes for all six targets. The `0.1.0a1` wheel was verified to contain exactly 28 MoirAI members—one manifest, four roles, eight skills, and fifteen knowledge files—with no `graph.yaml` or extra member; a clean Python 3.11 install read the `4/8/15` inventory and projected a temporary project through the wheel's `gskill`. On Windows `10.0.26200` x64, Claude Code `2.1.222` discovered the eight skills, four agents, and `gskill` MCP entry and connected the stdio server; Codex CLI `0.144.1` independently supplied `$moirai` and successfully called `gskill.inspect`. These observations do not prove authenticated Claude model execution, Codex custom-agent invocation, or operational support for all six products.

Phase 6 accepts one source-bound wheel/source-distribution candidate by validating its package contract and exercising pip-wheel, uv-wheel, and pip-sdist installations on Ubuntu, Windows, and macOS. The installed smoke imports from site-packages, proves that the distribution owns no console entry or launcher, invokes the isolated environment's Python with `python -m graph_skill_runtime`, verifies the read-only configuration guide's zero-write repeatability, and observes exactly eight MCP tools plus the runtime, durable handoff, SQLite, path, and explicit integration contracts. [`accept_release_artifacts.py`](../scripts/accept_release_artifacts.py) and its evidence schemas are repository release tooling, not members of the top-level Python facade or MCP tool set; the public contract therefore remains exactly 77 symbols, fourteen top-level Python functions (nine runtime/application plus five integration), and eight MCP tools. This pre-publication acceptance neither proves a PyPI/GitHub release nor expands real vendor-CLI support. Studio and Gateway plugins remain future external Port/Adapter concerns.

An earlier uncommitted `1.0.0a1` working tree passed Ruff, strict mypy over 155 source files, `1739 passed, 1 skipped`, contract/checklist tests, build, and three Windows installed-package channels on 2026-08-29. Those environments observed eight MCP tools, runtime, handoff, MoirAI, and an Agent-kit setup shape that has since been removed; they do not verify the current read-only guide/additive-placement contract. The artifacts contained uncommitted bytes even though acceptance recorded a HEAD value, so this remains historical local working-tree/package evidence, not current setup guidance, a commit-bound candidate, registry publication, or new three-platform acceptance.
