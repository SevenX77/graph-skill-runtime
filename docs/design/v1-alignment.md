...
本文定义把提取后的 engine 建成独立 Python runtime、SDK 与 CLI 的完整 v1 目标。它与 [`baseline.md`](./baseline.md) 双向绑定。本文整体状态保持 `drafted`：Phase 0、Phase 1、Phase 2 portable 文件格式、Phase 3 中 root DAG 串行可定位 Agent wait point 的 durable host-native handoff、Phase 4 direct vendor CLI executor、Phase 5 的 canonical MoirAI integration，以及历史 `0.1.0a1` Phase 6 同候选制品跨平台 package/release acceptance 已经按各自边界验收；`1.0.0a1` provider-neutral Agent kit 已在当前工作树实现并完成本地 package smoke。Phase 3b 的 host-native 扩展尚未实现，首次发布前命名裁决和真实 release/registry publication 也尚未闭合；当前 `1.0.0a1` 制品又包含未提交 bytes，不能视为新的 commit-bound 候选。因此不能把完整 v1 当作当前能力或已发布产品。Gateway 与 Studio plugin 不属于本轮 release 交付；本文只保留其未来外部 Port/Adapter 所有权边界。
...
| Section 2：产品命名 | **已实现于源码与仓库** | distribution/import/module CLI 是 `graph-skill-runtime` / `graph_skill_runtime` / `python -m graph_skill_runtime`，当前目标版本 `1.0.0a1`，版本输出同时报告 syntax `gskill.graph.v1`；release workflow 已实现单次 build、三平台 acceptance 与 OIDC publish gate，但 PyPI project/publisher 尚未配置，也没有实际发布 |
...
| Section 4 至 Section 5：portable 格式与 flat graph registry | **Phase 2 已实现并收紧 marker** | Production compile/run/SDK/CLI/MCP 只接受显式 root `SKILL.md` + `graph.yaml` bundle；两处必须声明匹配的 `gskill.graph.v1`，distribution/syntax major 必须相等；内部 agent phase 使用 `AGENT.md`；graph registry 为单层 `graphs/<graph_id>/`；legacy v0.3 读取只存在于显式 converter 边界 |
...
| Provider-neutral Agent kit | **当前工作树已实现；当前 contract 已重写** | 零写入 `guide agent-configuration` 返回 AGENTS section、十份 standalone rules 与两份完整 Skill/reference trees；owner 选择 host/scope/destinations/manual-or-Agent mode 后再批准 additive write；MCP-first/module-CLI-fallback 与 rule-05 native child handoff 已落地；没有 setup command，不新增 Python symbol、MCP tool、vendor plugin/profile，也不替代 MoirAI |
| Phase 6：跨平台 package/release acceptance | **历史 `0.1.0a1` 候选已验收于定义范围** | 一个 manifest-bound wheel/sdist 候选已在 Ubuntu、Windows、macOS 分别通过三安装通道；当前 `1.0.0a1` 只有未提交工作树的 Windows package smoke，不是新的 source-bound 或三平台 acceptance |
| Gateway/Studio integration | **不属于本轮 release** | 只保留未来外部 Port/Adapter 的 owner 边界；不以 plugin、product cutover 或真机旅程作为本轮完成项 |

当前公共 API 的精确事实源是 [`../public-api-contract.md`](../public-api-contract.md) 与 `src/graph_skill_runtime/__init__.py`。Provider-neutral Agent kit 的权威设计是 [`unified-agent-kit.md`](./unified-agent-kit.md)。当前 MoirAI inventory 的精确事实源是 `src/graph_skill_runtime/integrations/assets/moirai/integration.json`；renderer 与 installer 行为分别由 `integrations/renderers.py` 与 `integrations/installer.py` 拥有。当前文件格式的事实源是状态为 `audited-ready` 的 [`../skill-spec/01-PORTABLE-GSKILL-V1.md`](../skill-spec/01-PORTABLE-GSKILL-V1.md)；[`../skill-spec/00-FORMAT-GROUND-TRUTH.md`](../skill-spec/00-FORMAT-GROUND-TRUTH.md) 已被取代，只保留为 legacy converter 输入契约与历史证据。本设计后文保留完整 v1 目标；实现存在、验收通过、未来边界与未支持声明必须分开，不能互相替代。
...
2026-08-29 的早期 `1.0.0a1` 工作树曾通过 Ruff、155-source strict mypy、完整 pytest `1739 passed, 1 skipped in 136.85s (0:02:16)`、contract/checklist tests 与 `uv build --no-sources`。Windows 上三个隔离安装通道当时观察到八个 MCP tools、compile/predict/run、host-native 与 MoirAI lifecycles，以及后来被移除的 Agent-kit setup 形状。该历史证据不验证当前 read-only guide/additive-placement contract，也不能作为当前 setup 使用依据。构建还包含未提交 bytes；acceptance JSON 记录的 HEAD `230fc3bd...` 不能把这些 bytes 绑定到该 commit。因此它只是 historical local working-tree/package evidence，不是 publishable release candidate、registry publication 或新三平台 acceptance。
...
这些工作名已经在当前 repository、distribution metadata 与 Python import 中实现。当前 distribution 不定义 package console entry；installed-user CLI 使用 `python -m graph_skill_runtime`。GitHub repository `SevenX77/graph-skill-runtime` 已存在；PyPI 尚未发布，名称也不构成商标许可。repository 的 release workflow 已按 `v<pyproject version>` release tag 分离一次 build、三平台 verify 与 publish：只有 build 和三平台 source/installed-package acceptance 全部成功，publish job 才能下载原始 distributions，并通过 OIDC Trusted Publishing 请求上传。owner 仍须先在 PyPI 建立 project 与 trusted publisher。首次公开 registry 发布前还须完成占名与商标复核，若复核失败，应在发布前一次性裁决新名称，不能增加永久 alias。

当前 distribution target 为 `1.0.0a1`，portable syntax literal 为 `gskill.graph.v1`；两者是不同表示，但 major 必须一致。`python -m graph_skill_runtime --version` 精确输出 `python -m graph_skill_runtime gskill.graph.v1 (graph-skill-runtime 1.0.0a1)`。Implementation patch 或 alpha 版本变化不自动改变 portable syntax。
...
因此，module CLI `python -m graph_skill_runtime compile`、MCP `compile` 和 `graph_skill_runtime.compile(...)` 必须返回同源诊断；`run`、`resume` 与 `submit_agent_result` 也不得各自实现状态转换。

**Phase 1/4/5 与 Agent-kit 实现说明**：上述 domain/application/ports/adapters/integrations 分层与七十七个顶层 typed symbols 已实现。顶层 Python function 精确为 14 个：`create_application` 加八个 runtime SDK use cases 构成九个 runtime/application entry points，五个 integration SDK functions 组成其余五个。八个 runtime use cases、module CLI runtime commands 与八个 MCP runtime tools 共享同一个 `RuntimeApplication`；五个 integration SDK functions 与 module CLI `integrations` commands 共享显式 `IntegrationInstaller`，没有 installer MCP tool。Module CLI 另有 read-only `guide agent-configuration`、`create`、`migrate` 与 `mcp`；没有 unified-kit setup/install command。它们不增加 Python export 或 MCP tool。Phase 4 增加公共 `AgentResource`；Phase 5 增加十三个 integration model/installer names 与五个 integration SDK functions，合计 18 个新增顶层 names。Runtime 公共 Pydantic contracts 位于 `domain/models.py`，integration 公共 Pydantic contracts 位于 `integrations/models.py`；两者都是 closed、frozen、带 `schema_version` 与 `kind` 的对象，构造后的嵌套 JSON collection 也不可变。`create_application` 是显式 composition root，每次调用构造独立 application，不持有全局 singleton。
...
**当前实现边界**：Phase 2 已把本节设计落实为 production reader。当前 core 只接受显式 skill root 下的根 `SKILL.md`、`graph.yaml`、phase `LOGIC.md` / `AGENT.md` / `SUBGRAPH.md` 和单层 `graphs/<graph_id>/` registry；没有 dual reader。根 `SKILL.md` 必须含 `metadata.gskill: gskill.graph.v1`，`graph.yaml` 必须含 `schema_version: gskill.graph.v1`，缺失或不匹配以 `[F-v3-skill-metadata-invalid]` 失败。字段级格式与 converter 的当前权威是 [`../skill-spec/01-PORTABLE-GSKILL-V1.md`](../skill-spec/01-PORTABLE-GSKILL-V1.md)，本节说明完整 v1 设计中的职责与动机，不另建平行 schema。
...
业务 gSkill 的 root metadata 必须额外声明 `gskill: gskill.graph.v1`。这个 marker 识别 executable business root，不适用于普通 Agent Skill、provider-neutral operator kit Skill 或内部 `AGENT.md`。
...
`python -m graph_skill_runtime inspect SKILL_ROOT --call-graph` 从显式 call edge 生成面向人的调用拓扑。parent 和 callers 必须从 edge 派生，不能另存 `parent` 真相；同一 graph 可以被多个 caller 复用，单一 parent 字段无法表达这一事实。
...
5. 当前宿主 root agent 创建一个新的 native clean-context child，先使用 provider-neutral kit rule 05 唯一拥有的 canonical prompt，再提供完整 `AgentTask`；child 只返回一个满足 `task.output_schema` 的 JSON object；
6. 宿主通过 Python SDK `submit_agent_result`、同名 MCP tool 或 module CLI `python -m graph_skill_runtime submit` 提交 `AgentResult`；
...
若当前宿主没有 native subagent 能力，宿主必须明确报告不能执行，不能伪造 task completion。Optional clean-context delegation 只允许一个 bounded independent domain/topology/prompt、repair 或 evidence-judgement task，且必须在 isolation 确实有益时由当前宿主自动选择；用户不选择内部 specialist role。精确 prompt、ordering 与 delegation boundary 由 [`unified-agent-kit.md`](./unified-agent-kit.md) 统一拥有，本文不复制 prompt。当前 runtime 能把 `required_capabilities` 放入 task，但尚无 host-native acknowledgment 来证明 capability、`agent_dispatched` 或 `agent_started`；该协商属于 Phase 3b。无论宿主能力如何，runtime 都不静默选择 fallback executor。显式选择 `embedded` 才进入 embedded，显式选择 `cli` 才进入下一节的 runtime-owned direct vendor process 路径。
...
新仓不实现 dual reader 或 legacy shim。当前一次性转换入口是 `python -m graph_skill_runtime migrate studio-skill SOURCE DESTINATION [--runtime-config PATH] [--preset-id ID]`：它显式读取旧 Studio skill 与 runtime config，生成 portable 目录、project `gskill.toml`、named migration preset 与确定性迁移报告。它先完成只读 preflight，在 destination 的 sibling temp 目录生成，再以操作系统原生 create-if-absent rename 发布；未知或有冲突字段、无法平铺的 nested subskill、source/destination 冲突或 existing destination 都会在不修改 source、不发布 partial destination的前提下失败。
...
Canonical skill 的 YAML frontmatter 只含 `name` 与 `description`，其余正文按 progressive disclosure 链接普通相对 Markdown references。Role source 只含 provider-neutral body；renderer 再生成宿主 metadata。Canonical prompt 只调用属于 `gskill` MCP server 的公开 runtime tools，或在 MCP 不可用时调用 installed interpreter 的 `python -m graph_skill_runtime` module CLI；不得依赖 source checkout 或 package-owned launcher。

安装 distribution、`import graph_skill_runtime`、构造 installer、启动 MCP 与 PATH detection 都不修改宿主或项目。唯一写入授权是显式 `python -m graph_skill_runtime integrations install moirai ...` 或等价 SDK `install_integration(IntegrationRequest(...))`。`detected` 只是 PATH read evidence；只有把它放入显式 install command 后才成为目标选择。Installed-user 流程先 detect、再 dry-run、再 apply：

```text
python -m graph_skill_runtime integrations detect
python -m graph_skill_runtime integrations install moirai --targets codex --scope user --dry-run
python -m graph_skill_runtime integrations install moirai --targets codex --scope user
python -m graph_skill_runtime integrations uninstall moirai --targets codex --scope user --dry-run
python -m graph_skill_runtime integrations uninstall moirai --targets codex --scope user
```
...
CLI fallback 只使用 installed interpreter 的 module CLI：`python -m graph_skill_runtime compile SKILL_ROOT`、`python -m graph_skill_runtime predict SKILL_ROOT [--inputs-json JSON]`、`python -m graph_skill_runtime run SKILL_ROOT [--inputs-json JSON]`、`python -m graph_skill_runtime inspect SKILL_ROOT --call-graph`、`python -m graph_skill_runtime golden SKILL_ROOT BASELINE_ID --state-root STATE_ROOT`、`python -m graph_skill_runtime resume SKILL_ROOT RUN_ID --state-root STATE_ROOT --checkpoint-ref REF`、`python -m graph_skill_runtime submit RUN_ID --state-root STATE_ROOT --checkpoint-ref REF --result-json JSON`。Canonical assets 不使用 bare `gskill`、`uv run` 或 source checkout，也不复制 compile/run 判据。
...
- Python SDK、MCP `submit_agent_result` 与 module CLI `python -m graph_skill_runtime submit` 到达同一 application service；CLI 把 `agent_required` 当作成功的两步协议状态并返回 exit code 0；
...
**验收证据**：`tests/application/test_host_native_handoff.py` 的 12 条 manifest-targeted tests 覆盖跨进程闭环、多个串行 Agent phase 每次只暴露一个 durable task、非法输出纠正、cancelled terminal failure、篡改 request snapshot 拒绝、graph-commit/handoff-response crash window 与 graph-pause/handoff-row crash window，并另有 durable SQLite、parallel fail-fast、secret-shaped durable result rejection、CLI 两步协议与 MCP 同源提交证据。Ruff green，strict mypy 覆盖 135 个 source files green，manifest validator green，包含全部 12 条 Phase 3 targeted evidence 的本地完整 pytest suite 为 `1604 passed / 1 skipped in 83.88s`。一次历史人工实测使用当时存在、现已移除的 bare run/submit 命令，让同一 `run_id=real-host-closure` 完成，`executor_id=codex-native-subagent`。该命令形状只保留为历史证据，不是当前使用说明；本记录也不把机器本地临时 state 路径冒充长期 CI 证据。
...
Windows `10.0.26200` x64 / Python `3.11.15` 上，Codex CLI `0.144.1` 的一条历史真实运行以 `run_id=codex-post-hardening-smoke` 完成，task、attempt 与 vendor session id 见本文 Section 0，输出为 `echoed_note="post hardening verified"`。该证据使用当时存在、现已移除的 bare launcher，不能作为当前命令示例。Claude Code `2.1.222` 的 auth-missing 与不存在 Copilot executable 的两次真实负向 smoke 都在 handoff DB 创建前返回结构化 `GSKILL_EXECUTOR_UNAVAILABLE`。它们证明 fail-before-handoff，不证明成功支持。其他四 vendor 未安装，macOS/Linux real vendor execution 未实测。
...
- `IntegrationInstaller` 与五个 SDK functions、module CLI `integrations detect/install/uninstall` 已落地；construction、import、MCP startup 与 detection 零写入，只有 explicit install request 授权 host/project mutation；
...
### Provider-neutral Agent kit：通用宿主操作层

**当前状态（2026-08-29）**：已实现于当前工作树。权威设计是 [`unified-agent-kit.md`](./unified-agent-kit.md)；本节只记录它与完整 v1 的边界。

- `python -m graph_skill_runtime guide agent-configuration` 是零写入 source/placement guide，`python -m graph_skill_runtime create NAME --path EXISTING_PARENT --description TEXT` 是 scaffold operation；不存在 unified-kit setup/install command。两者不增加 Python export 或 MCP tool；MCP 仍精确八个 runtime tools，module CLI 是 MCP unavailable 时的正式 fallback。
- kit 只有 `gskill` 与 `create-gskill` 两个 automatic Agent Skills。前者只操作带 `metadata.gskill: gskill.graph.v1` 的 existing business root；后者只处理 create/scaffold/convert/structural redesign，不接管 ordinary Agent Skill authoring。
- guide 返回 provider-neutral AGENTS section、十份 standalone indexed rules 与两份完整 Skill/reference trees；owner 选择 host、user/project scope、rules destination、Skill destinations 与 manual/Agent-authorized mode。写入前须 inspect、提出 exact additive plan 并获批；不得替换既有 instruction file。package install 与 guide 均不自动选择或写入目标。
- 每个 host-native `agent_required` 使用一个 fresh native clean-context child；canonical prompt 只由 rule 05 拥有。Optional delegation 仅限一个 bounded independent domain/topology/prompt、repair 或 evidence-judgement task。用户不需要也不应选择内部 role。
- kit 不含业务 `graph.yaml`，不注册用户 gSkill，也不新增 vendor plugin/profile。MoirAI 仍是上节的独立可选 host projection。

### Phase 6：跨平台 packaging 与 release acceptance

**历史 `0.1.0a1` 状态（2026-08-28）**：已在“同一 built candidate 的跨平台安装与发布前验收”范围内验收。该状态表示验收 evidence 绑定的 wheel/sdist 候选满足已定义的 package contract，并且 release workflow 只允许已通过三平台验证的原始 bytes 进入 publish job；它不表示已经创建 tag/GitHub Release、配置 PyPI project/trusted publisher，或向 PyPI/TestPyPI 上传。任何后续重建都是新候选，必须生成新 manifest 并重新验收。Phase 4 的成功 direct-vendor operational statement 仍只有 Windows/Codex 一项。

**已落实工作**：

- [`scripts/accept_release_artifacts.py`](../../scripts/accept_release_artifacts.py) 的 `validate` 要求 dist directory 恰好包含一个 wheel 和一个 `.tar.gz` sdist，检查 metadata、`py3-none-any` 与安全 archive path，并要求不存在 `[project.scripts]`、`console_scripts` 或 package-owned launcher（包括 `Scripts/gskill.exe` 与 `bin/gskill`）；wheel 明确拒绝 symlink，sdist member 必须是 regular file 或 directory。Wheel 拒绝 `graph_agent/` 与 `graph_skill_runtime/examples/`，sdist 拒绝 `src/graph_agent/` 与 `src/graph_skill_runtime/examples/`；两者都对 provider-neutral Agent kit 和既有 MoirAI assets 执行 closed inventory，并拒绝 extra kit member 与 installed business `graph.yaml`。Sdist 可以保留 repository-level examples/tests 作为源码语料，但这些内容不会进入 installed distribution 或注册到宿主。这不是整个 runtime archive 普通源码 member 的全量白名单。输出 `gskill.release-artifacts.v1` 绑定 supplied source commit、文件名、size 与 SHA-256。
...
- internal `installed-smoke` 从 site-packages 导入且拒绝 base install 混入 provider extras、`graph_skill_runtime/examples/`、任何 business `graph.yaml` 或 package-owned launcher；检查 version、module CLI identity 与只读 host detection；用隔离环境的 Python 调用 `python -m graph_skill_runtime`，验证 compile/inspect/predict/run 与空格/非 ASCII path；验证 `guide agent-configuration` 零写入且重复调用结果稳定；通过真实 stdio 枚举精确八个 MCP tools 并调用 `compile`；同时覆盖 scaffold create、MoirAI project lifecycle、host-native run→reopen→submit→duplicate submit→terminal reopen、SQLite integrity/rename/reopen、immutable request/trace、Windows handle release 与零意外宿主状态改动。
...
**历史 `1.0.0a1` 本地补充证据（不构成新 Phase 6 acceptance）**：2026-08-29 的未提交工作树通过当时的 gates 与 Windows 三安装通道；后者观察到八工具 MCP、runtime/handoff/MoirAI，以及后来被移除的 Agent-kit setup 形状。该记录不验证当前 read-only guide/additive-placement contract。Evidence interface 记录 HEAD `230fc3bd...`，但 built artifacts 包含 uncommitted bytes；因此没有有效 commit binding，不能升级为 publishable candidate 或三平台结论。

**退出判据（已满足）**：一个 source-bound candidate 在三平台从 built artifact 而非 source tree 安装，并证明 import、无 console entry/launcher、module CLI identity 与 execution、guide 零写入重复性、精确八工具 MCP surface、durable handoff/reopen、SQLite、路径与 integration ownership 行为；wheel 与从 wheel/sdist 构建出的 installed distribution 不含用户业务 gSkill。源码 sdist 可以携带 repository-owned example/test corpus，但安装过程不得投影或注册它。CI 与 release workflow 都阻止未通过全部 verifier 的候选进入 publish job。Installed smoke 使用 deterministic host-native submission，不是 real vendor CLI；因此本阶段不要求也不推导 macOS/Linux vendor operational support。
...
2. 一个业务 gSkill 只有带 `metadata.gskill: gskill.graph.v1` 的根 `SKILL.md` 被宿主发现，机器 topology 和具名 artifact declarations 只有 `schema_version: gskill.graph.v1` 的 `graph.yaml` 定义，agent phase 只使用 `AGENT.md`；RunRequest 只按稳定 ID 请求已声明 artifact。
3. 编译聚合报告 graph/phase id、unknown reference、duplicate 和 cycle；`python -m graph_skill_runtime inspect SKILL_ROOT --call-graph` 与实际 call edges 同源。
...
8. `python -m graph_skill_runtime integrations install moirai` 的 dry-run、all-target conflict、manifest、因果安全 rollback、idempotency 与 safe uninstall 通过；六 renderer snapshots、built-wheel canonical inventory 与真实宿主 skill/agent/MCP discovery smoke 均有证据；显式安装前不投影宿主，安装资产也不包含用户业务 gSkill。Phase 5 已在当前定义范围满足本项，完整 v1 仍受其余未满足项约束。
...
11. Provider-neutral Agent kit 的 selected-target conflict/idempotence、两个 automatic Skills、MCP-first/CLI-fallback、safe create 和 rule-05 fresh native child handoff 通过；它不安装 vendor plugin/profile，不要求用户选择内部 role，也不替代 MoirAI。
...
| 日期 | 修订 | 依据与边界 |
| --- | --- | --- |
| 2026-08-29 | 当日先把分发目标改为 `graph-skill-runtime 1.0.0a1`，保留便携语法字面量 `gskill.graph.v1` 并要求二者 major 一致；可执行业务 gSkill 根 `SKILL.md` 必须声明 `metadata.gskill: gskill.graph.v1`；同日早期实现曾包含后来移除的 console launcher 与 setup command。当前修订已用 read-only `guide agent-configuration`、owner-approved additive placement 与 installed-interpreter module CLI 取代那些早期形状 | 运行时实现版本与便携语法版本是不同表示，需要独立演进但保持 major 兼容；业务 gSkill 需要可判定的显式身份；配置目的地与写入授权必须由 owner 选择。2026-08-29 的 1.0.0a1 早期证据只绑定本地未提交工作树与其构建包，且其中 launcher/setup 观察已被当前契约取代；它不替代 commit-bound release candidate、registry publication 或新一轮三平台验收 |
