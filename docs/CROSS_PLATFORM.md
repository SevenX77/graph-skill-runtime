...
- `installed-smoke` is the internal per-channel worker. The isolated environment's Python executes the worker, and `graph_skill_runtime` must resolve inside that environment rather than the checkout. It rejects provider extras, installed `graph_skill_runtime/examples/`, and installed `graph.yaml`; checks distribution/version/console identity and the six-target read-only host detector; opens a real stdio MCP session, enumerates the eight public runtime tools with their exact annotations, and successfully invokes MCP `compile`. It exercises the installed CLI's compile/inspect/predict/run behavior with spaces and non-ASCII text, and the installed MoirAI CLI's Claude/Codex project lifecycle `planned → installed → unchanged → uninstalled`. On Windows it also applies the CP936-hostile environment and strict UTF-8 assertion defined above.
...
The distribution under test was `graph-skill-runtime` `0.1.0a1`, whose `[project.scripts]` entry maps `gskill` to `graph_skill_runtime.adapters.cli:main`. On the Windows evidence host, `uv tool` produced `C:\Users\test\.local\bin\gskill.exe`, a 46,080-byte console-script launcher; the imported runtime package remained in the tool environment's `site-packages`. The change did not replace or embed the package into that launcher.
