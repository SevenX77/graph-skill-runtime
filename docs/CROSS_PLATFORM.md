...
| CLI and MCP output producer | [`main`](../src/graph_skill_runtime/adapters/cli.py) reconfigures each reconfigurable standard-output and standard-error stream to UTF-8 with strict errors before argument parsing or MCP startup |
| Python child-process backstop | General tests and CI set `PYTHONUTF8=1`; Windows installed-console acceptance deliberately overrides that backstop with `PYTHONUTF8=0` and `PYTHONIOENCODING=cp936` |
...
### Console and MCP stdio transport

The installed-user command is the selected interpreter running `python -m graph_skill_runtime`; the distribution defines no package-owned console launcher. Before argument parsing or MCP work, `main` sets every standard-output and standard-error stream that supports reconfiguration to UTF-8 with strict error handling. This defines the producer side of both structured CLI JSON and MCP stdio output. The emitted bytes must not vary with the Windows active code page, `PYTHONUTF8`, or `PYTHONIOENCODING`; strict output also prevents silent replacement from turning valid structured data into different text.

On Windows, hosts running `python -m graph_skill_runtime mcp` or another long-lived process from an environment can keep that environment's interpreter or `Scripts` directory open and block an in-place environment upgrade. Stop or restart every host using that environment before replacing it. A sharing violation in this state is an operational lifecycle conflict, not by itself a runtime installation defect; use a separate isolated environment for non-disruptive verification when other tasks must remain running.

Windows release acceptance must test the installed environment's interpreter with `python -m graph_skill_runtime`, not `uv run` from the source checkout. The acceptance process sets `PYTHONUTF8=0` and `PYTHONIOENCODING=cp936`, captures output as bytes, decodes those bytes with strict UTF-8, parses the structured result, and asserts that the non-ASCII value `全局安装` is preserved. A successful console display, a zero exit code, or a decode using replacement errors does not satisfy this contract.

The real MCP acceptance session must use that same installed interpreter with `python -m graph_skill_runtime mcp` over stdio. Its `tools/list` response must contain exactly eight tools and the explicit ToolAnnotations contract owned by the [public API contract](public-api-contract.md). The session must also verify the declared inspection behavior against observable state. These checks distinguish a connected transport from truthful host-visible side-effect metadata and read-only query behavior.
...
- `validate` requires the distribution directory to contain exactly one wheel and one `.tar.gz` source distribution. It validates distribution/version/`Requires-Python` metadata, requires the absence of `[project.scripts]` and `console_scripts`, rejects package-owned launchers including `Scripts/gskill.exe` and `bin/gskill`, checks the wheel's `py3-none-any` pure-Python declaration, and validates portable archive names. It rejects wheel symlinks; each source-distribution member must be a regular file or directory. The wheel rejects `graph_agent/` and `graph_skill_runtime/examples/`; the sdist rejects `src/graph_agent/` and `src/graph_skill_runtime/examples/`. Both require the exact closed provider-neutral Agent-kit inventory and the existing MoirAI inventory derived from packaged manifests; extra kit members and business `graph.yaml` fail validation. The sdist may contain repository-level examples and tests as source corpus; it does not install them as package content. The validator does not freeze every ordinary runtime source member into one full-archive whitelist. It writes `gskill.release-artifacts.v1` with the supplied exact source commit and the two artifacts' filenames, positive sizes, and SHA-256 values.
...
- `installed-smoke` is the internal per-channel worker. The isolated environment's Python executes the worker, and `graph_skill_runtime` must resolve inside that environment rather than the checkout. It rejects provider extras, installed `graph_skill_runtime/examples/`, installed business `graph.yaml`, and package-owned launchers; checks distribution/version/module-CLI identity and the six-target read-only host detector; opens a real stdio MCP session, enumerates exactly the eight public runtime tools with their annotations, and successfully invokes MCP `compile`. It invokes the isolated environment's Python with `python -m graph_skill_runtime` to exercise compile/inspect/predict/run behavior with spaces and non-ASCII text; verifies that `guide agent-configuration` is zero-write and repeatable; exercises scaffold creation and the installed MoirAI project lifecycle `planned → installed → unchanged → uninstalled`. On Windows it also applies the CP936-hostile environment and strict UTF-8 assertion defined above.
...
These choices prove provenance and pre-publication behavior, not registry state. A later release must still use the exact accepted bytes. Creating a tag, publishing a GitHub Release, configuring the PyPI project/trusted publisher, and observing the registry upload are external actions. None occurred for the historical accepted `0.1.0a1` candidate, and none has occurred for the current `1.0.0a1` target.
...
### Windows installed-host interoperability evidence — 2026-08-28

**Historical evidence only:** the distribution under that 2026-08-28 test was `graph-skill-runtime` `0.1.0a1`, which then defined a package console entry. On the Windows evidence host, the packaging tool produced a launcher while the imported runtime package remained in the tool environment's `site-packages`. The current `1.0.0a1` contract removed that entry; this observation is not current usage guidance.

Three pre-change observations establish causality: all eight tools in a real MCP `tools/list` response had `annotations=None`; `inspect` created the global compile-cache directory; and output initialized with CP936 could not be decoded as strict UTF-8. After the source change, the three targeted regression tests passed, as did 26 related tests. Full local validation then passed Ruff, strict mypy over 149 source files, the contract-manifest validator, and pytest with `1718 passed, 1 skipped`; `uv build` also succeeded. `pip-audit` reported no known vulnerabilities among resolved distributions but skipped the unpublished local distribution, so that result is not a source-code audit or publication evidence.

This local record verifies the source transport fix, MCP annotation contract, read-only inspection behavior, and ability to build distributions. The same newly built wheel was installed globally with `uv tool install --force <wheel>`. That installation form was valid only because the tested `0.1.0a1` distribution exposed a console command; it is a historical action, not current installation guidance. Current `1.0.0a1` defines no `[project.scripts]` or `console_scripts`, so it must not be installed with `uv tool install`: install it as a dependency into the owning Python or project environment, and use that environment's interpreter with `python -m graph_skill_runtime ...` for CLI fallback. Codex CLI `0.144.1`, without an approval override or allowed-tools whitelist, ran `codex exec --ephemeral -s read-only`, invoked `gskill.inspect` with `include_call_graph=true`, returned `skill_id=hello-world`, and exited `0`. This verifies that the real Codex host consumed the installed wheel's read-only ToolAnnotations and allowed `inspect` through its default approval path. Claude Code `2.1.250` still reported `Connected` through `claude mcp get gskill` and `claude mcp list`, but `claude -p` failed before any tool call because the local OAuth session had expired and could not be refreshed; Claude authenticated execution and its default tool call therefore remain unverified. These host observations are separate from the earlier immutable three-platform candidate above and do not expand any other host claim.

### `1.0.0a1` local working-tree/package smoke — 2026-08-29

The current uncommitted working tree passed Ruff, strict mypy over 155 source files, the full pytest suite (`1739 passed, 1 skipped in 136.85s (0:02:16)`), and the contract/checklist tests. `uv build --no-sources` created the `1.0.0a1` wheel and source distribution. The updated archive validator required both the closed provider-neutral Agent kit and the existing closed MoirAI inventory, and rejected extra kit members and installed business `graph.yaml`.

On Windows, the earlier pip-wheel, uv-wheel, and pip-sdist isolated environments imported `graph_skill_runtime`; exposed exactly eight MCP tools; passed compile and predict/run; observed host-native and MoirAI lifecycles; and reported no unexpected host-state changes. They also observed an Agent-kit setup shape that has since been removed. This historical evidence does not validate or authorize the current read-only guide/additive-placement contract.

The acceptance JSON interface recorded current HEAD `230fc3bd...`, but the artifacts were built from an uncommitted working tree. That recorded value does not bind the artifact bytes to the commit. These results are therefore local working-tree/package smoke, not a publishable commit-bound release candidate, tag, registry publication, or new Ubuntu/Windows/macOS acceptance.

**Historical evidence only:** an in-place tool reinstall into the previously used Windows environment was blocked while live hosts still used that environment. Verification used a second isolated directory. The then-current launcher/setup/create/compile checks passed. Those removed command shapes are preserved only as dated evidence and must not be used for current `1.0.0a1` operation.
...
- installed Windows CLI acceptance disables Python UTF-8 mode, requests CP936, strict-decodes captured bytes as UTF-8, and preserves the required non-ASCII value;
- a real installed MCP `tools/list` session returns the exact eight annotated tools, and `inspect` leaves the global compile-cache directory absent;
...
- archive and installed-package checks enforce both closed instruction-asset inventories while rejecting user business `graph.yaml`;
- a working-tree build is not described as commit-bound merely because its evidence interface recorded current HEAD;
