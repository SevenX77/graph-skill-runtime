...
This local record verifies the source transport fix, MCP annotation contract, read-only inspection behavior, and ability to build distributions. It is separate from the earlier immutable three-platform candidate above. A default MCP tool call from either current host after installing the newly built wheel has not yet been observed and must remain unverified until that installed-host action succeeds.
