    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            for alias in node.names:
                if alias.name == "graph_skill_runtime.cognitive.context_facade":
                    form = "form 3" if alias.asname else "form 2"
                    violations.append(
                        PurityViolation(
                            path=path,
                            line=node.lineno,
                            api="graph_skill_runtime.cognitive.context_facade",
                            reason=f"Tools must not import the Action Context facade ({form})",
                        )
                    )
        elif (
            isinstance(node, ast.ImportFrom)
            and node.module == "graph_skill_runtime.cognitive.context_facade"
        ):
            violations.append(
                PurityViolation(
                    path=path,
        ...
        elif isinstance(node, ast.ImportFrom) and node.module == "graph_skill_runtime.cognitive":
            for alias in node.names:
...
def _violation_for_call(
    ...
    if full_name is not None:
        if full_name in {"run_skill", "graph_skill_runtime.run_skill", "graph_skill_runtime.core.runner.run_skill"}:
