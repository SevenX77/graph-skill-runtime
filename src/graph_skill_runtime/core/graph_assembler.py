...
from graph_skill_runtime.callbacks.emit import (
    _GatewayEventSink,
    _safe_emit_event,
...
from graph_skill_runtime.cognitive.critic import (
    CriticVerdict,
    FakeCriticClient,
    LLMCriticClient,
    build_critic_tool,
)
from graph_skill_runtime.cognitive.finish_task import build_finish_task_tool
from graph_skill_runtime.cognitive.md2json import parse_finish_markdown
from graph_skill_runtime.cognitive.md_patch import LLMMdPatchClient
...
from graph_skill_runtime.core.actions import ToolDef, _structured_tool
from graph_skill_runtime.core.builtin_subagents import ReferenceReaderRuntime
...
from graph_skill_runtime.core.llm_provider import ChatModelProvider, LLMProvider, LLMProviderChatModel
from graph_skill_runtime.core.loader import CompiledSkill, CompiledSubagent, PhaseDocument, SkillLoader
...
from graph_skill_runtime.core.subagents import (
    SubagentValidationFailure,
    assert_subagent_depth_allowed,
    current_subagent_depth,
    validate_subagent_tool_args,
)
from graph_skill_runtime.core.validator_contract import VALIDATOR_SIGNATURE
...
from graph_skill_runtime.tools.builtin.cognitive_tools import (
    log_ambiguity_tool,
    query_working_memory_tool,
    read_artifact_tool,
    update_working_memory_tool,
)
from graph_skill_runtime.tools.builtin.read_example import read_declared_example
from graph_skill_runtime.tools.builtin.read_file import (
    RuntimeInputFileError,
    list_workspace_batch_files,
    read_workspace_text_file,
)
from graph_skill_runtime.tools.builtin.read_reference import read_declared_reference, read_resource_file
...
def _phase_outputs_delta(
    ...
    *,
    ...
    Token counters are deliberately NOT here (OB10). They used to ride along
    for the same reason, and it never worked: the flow channel folds a dict
    delta per key by OVERWRITING, so when several phases write in one superstep
    only one survives. What a run spent is now counted as each call reports
    itself, on the run's event sink, where a channel cannot drop it.
    """
    ...
    return {
        "data": data_delta,
        "flow": {"phase_execution_ids": phase_execution_ids},
    }
    ...
    *,
    ...
    def _loop_phase(state: WorkflowState) -> dict[str, Any]:
        ...
        for index, item in enumerate(items, start=1):
            child_state = StateManager.update_business( loop_state, **{iterate.item_var: item, accumulate.var: acc}, )
...
def _load_phase_validator(
    *,
    ...
    module_name = f"_graph_skill_runtime_validator_{abs(hash(validator_path.resolve()))}"
...
def _build_skill_node(
    ...
    *,
    ...
    from graph_skill_runtime.middleware.compaction import write_compaction_sidecar
    ...
    def _skill_node(
        ...
        # Tool calls are reported from here because they really are already
        # over by the time the inner graph returns. Token spend is NOT counted
        # here (OB10): a reader of a finished message list can only describe
        # what remained, and the run's ledger counts each call as it happens.
...
def _resolve_phase_chat_model(
    ...
    *,
    ...
    # User ruling 2026-08-31: the runtime invents no fallback role, and every
    # seam below declares the role in its contract — the predict stub records it,
    # the provider request resolves models by it, the call step labels events
    # with it. A phase that resolves no role cannot honestly reach any of them,
    # so it fails here with the fix spelled out. In practice nothing gets this
    # far, because compile rejects it unconditionally as
    # [F-v3-agent-llm-role-missing]; this guard covers callers that assemble a
    # hand-built AST without going through the compiler.
    if llm_role is None:
        raise ValueError(
            f"Agent phase {phase_id!r} resolves no LLM role: set `llm_role` in the phase "
            "frontmatter or a graph default `llm_role` in that graph's graph.yaml (a registry "
            "graph declares its own default; the calling graph's does not reach inside it)."
        )
