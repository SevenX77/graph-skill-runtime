"""Exception hierarchy for graph_skill_runtime framework.

All graph_skill_runtime errors inherit from GraphAgentError. Catch the most
...
class ErrorPayload(BaseModel):
    ...
    def _fill_registry_metadata(self) -> ErrorPayload:
        ...
        if metadata is None:
            raise ValueError(f"unknown graph_skill_runtime error code: {self.code}")
...
def _payload_from_message(message: str) -> ErrorPayload | None:
    ...
    if code not in ERROR_REGISTRY:
        if code.startswith(_EXTERNAL_ERROR_CODE_PREFIXES):
            return None
        raise ValueError(f"unknown graph_skill_runtime error code in message: {code}")
    return ErrorPayload(code=code, message=message)


class GraphAgentError(Exception):
    """Base for all graph_skill_runtime framework errors.
...
class GraphAgentFatalError(GraphExecutionError):
    """Fail-fast graph_skill_runtime error for violated hard invariants."""
...
class SchemaValidationError(ValidationError):
    ...
    when crossing graph_skill_runtime boundaries.
    """
...
class ToolExecutionError(GraphExecutionError):
    ...
    through graph_skill_runtime's exception family. Tool wrapper code should catch this
