...
The compiler resolves every call, rejects unknown ids and call cycles, and derives callers from edges. `gskill inspect SKILL_ROOT --call-graph` projects the compiled edge set. Do not store a separate `parent` or callers list.
