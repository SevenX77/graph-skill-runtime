...
The 2026-08-28 Windows CLI/MCP interoperability change has separate local causal evidence. Before the change, the regressions observed eight missing tool annotations, an inspection-created cache directory, and CP936-initialized output that failed strict UTF-8 decoding. After the change, the three targeted tests and 26 related tests passed; Ruff, strict mypy over 149 source files, the contract manifest validator, the full suite (`1718 passed, 1 skipped`), and `uv build` also passed. `pip-audit` found no known vulnerabilities among resolved distributions and skipped the unpublished local distribution. This evidence validates the changed source and build, not a new three-platform accepted candidate or a default host tool call from the newly built wheel.
