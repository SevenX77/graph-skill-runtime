...
The optional MoirAI integration is a separate host-projection domain. [`integration.json`](src/graph_skill_runtime/integrations/assets/moirai/integration.json) owns its exact asset version, four role identities and host names, eight skill ids, fifteen knowledge filenames, and each skill's copied reference subset. Canonical role bodies, skill instructions, and knowledge live only under that manifest's asset root and contain no business `graph.yaml`. Renderers adapt that one bundle to host-native skill, agent, and MCP configuration formats; they do not create a Studio plugin, Gateway plugin, dedicated Claude plugin bundle, or global business-skill registry. Canonical and provider-neutral specialist names remain hyphenated. The Codex adapter alone normalizes each projected agent identifier to underscores, for example `.codex/agents/moirai_clotho.toml` with `name = "moirai_clotho"`; the other host projections retain the manifest's hyphenated names.
...
Real-host observation on Windows `10.0.26200` x64 completed the scoped discovery exit. Claude Code `2.1.222` discovered all eight project skills, all four projected agents, and the project `gskill` MCP entry, then started and connected that stdio server; the valid `moirai-clotho` selection passed profile discovery and stopped only at missing authentication. This proves discovery and MCP startup, not authenticated Claude model execution. Codex CLI `0.144.1` independently supplied the projected `$moirai` skill and used project MCP tool `gskill.inspect` successfully, but its observed spawn surface exposed no `agent_type`; the child recorded `agent_role=null` and did not load the custom `developer_instructions`. Codex custom-agent invocation is therefore not verified, and this observed runtime limit does not invalidate its documented standalone TOML format. These bounded observations accept Phase 5 without claiming that all six host products are operational. Gateway and Studio integrations remain future external Port/Adapter concerns and are outside this release line.
...
`accept` verifies the artifact sizes and SHA-256 values recorded in that manifest, checks `--expected-source-commit`, copies the verified bytes, and installs pip-wheel, uv-wheel, and pip-sdist channels in isolated environments. It writes `gskill.package-acceptance.v1`, including the SHA-256 of the manifest it consumed. There is no external expected-manifest-hash parameter; equal manifest digests in independently produced platform evidence establish that those platforms consumed the same manifest. The installed smoke imports from site-packages, excludes provider extras and installed business examples/`graph.yaml`, checks version/console/detection, enumerates exactly eight MCP tools over real stdio and calls MCP `compile`, exercises installed CLI compile/inspect/predict/run, spaces/non-ASCII paths, Claude/Codex project projection, durable host-native reopen/submit/idempotency, SQLite integrity/rename/reopen, immutable request/trace presence, Windows handle cleanup, and absence of unexpected host-state writes outside the owned compile cache.
