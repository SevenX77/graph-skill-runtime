...
class TestIOManagerArtifactTargetAlignment:
    def test_save_outputs_accepts_target_artifact(self, tmp_path: Path) -> None:
        ...
        io_mgr = IOManager({"outputs": [{"name": "story_framework", "target": "artifact"}]})
    ...
    def test_save_outputs_still_accepts_legacy_artifact_manager(self, tmp_path: Path) -> None:
        ...
        io_mgr = IOManager({"outputs": [{"name": "story_framework", "target": "artifact_manager"}]})
    ...
    def test_artifact_saver_exception_propagates(self) -> None:
        ...
        assert io_mgr.io_errors == [
            "artifact_saver failed for 'story_framework': disk full while saving story_framework"
        ]
    ...
    def test_artifact_target_without_saver_raises_and_records_io_error(self) -> None:
        ...
        assert "_io_errors" not in context
        assert io_mgr.io_errors == ["Artifact target output 'story_framework' has no saver"]
