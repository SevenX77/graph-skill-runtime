...
def test_root_persona_skill_file_is_rejected(tmp_path: Path) -> None:
...
def test_subskills_persona_does_not_make_v21_root_valid(tmp_path: Path) -> None:
...
def test_absolute_persona_file_path_is_not_a_v21_skill_root(tmp_path: Path) -> None:
    ...
    with pytest.raises(SkillLoadError, match="expects a skill root directory"):
        SkillLoader().compile_skill(persona)
